...
final class WPD {
    ...
    private static PortableDeviceKeyCollection keyCollection;
    ...
    static PortableDeviceObject convertToPortableDeviceObject(String objectID, 
    	...
    	try {
            ...
            if (contentType.equals(Win32WPDDefines.WPD_CONTENT_TYPE_FOLDER)) {
                return new PortableDeviceFolderObject(objectID, content, properties);
            	...
            	if (category.equals(Win32WPDDefines.WPD_FUNCTIONAL_CATEGORY_STORAGE)) {
            		return new PortableDeviceStorageObject(objectID, content, properties);
            	} else if (category.equals(Win32WPDDefines.WPD_FUNCTIONAL_CATEGORY_RENDERING_INFORMATION)) {
            		return null;
            	} else {
            		return new PortableDeviceObject(objectID, content, properties);
            	}
            } else if (contentType.equals(Win32WPDDefines.WPD_CONTENT_TYPE_AUDIO)) {
                return new PortableDeviceAudioObject(objectID, content, properties);
            } else if (contentType.equals(Win32WPDDefines.WPD_CONTENT_TYPE_PLAYLIST)) {
                return new PortableDevicePlaylistObject(objectID, content, properties);
            } else {
                return new PortableDeviceObject(objectID, content, properties);
            }
