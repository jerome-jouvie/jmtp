...
#include "jmtp.h"
#include "jmtp_PortableDeviceKeyCollection.h"

static inline IPortableDeviceKeyCollection* GetPortableDeviceKeyCollection(JNIEnv* env, jobject obj) {
	return (IPortableDeviceKeyCollection*)GetComReferencePointer(env, obj, rt.pdKeyCollectionField);
}
...
JNIEXPORT void JNICALL Java_jmtp_PortableDeviceKeyCollection_add
	...
	if (jobjKey == NULL) {
		env->ThrowNew(rt.illegalArgEx, "key can't be null");
		return;
	} else {
...
JNIEXPORT jobject JNICALL Java_jmtp_PortableDeviceKeyCollection_getAt
	...
	if (jlPosition < dwCount && jlPosition >= 0) {
		...
		env->ThrowNew(rt.ioobEx, "Invalid index");
		return NULL;
	}
