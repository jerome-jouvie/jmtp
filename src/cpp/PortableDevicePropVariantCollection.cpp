...
#include "jmtp.h"
#include "jmtp_PortableDevicePropVariantCollection.h"

static inline IPortableDevicePropVariantCollection* GetPortableDevicePropVariantCollection(JNIEnv* env, jobject obj) {
	return (IPortableDevicePropVariantCollection*)GetComReferencePointer(env, obj, rt.pdPropVariantCollectionField);
}
...
JNIEXPORT void JNICALL Java_jmtp_PortableDevicePropVariantCollection_add
	...
	if (jobjPropVariant != NULL) {
		...
		env->ThrowNew(rt.npeEx, "propvariant isn't allowed to be null");
	}
}
...
JNIEXPORT jobject JNICALL Java_jmtp_PortableDevicePropVariantCollection_getAt
	...
	if (SUCCEEDED(hr)) {
		if (jlIndex >= 0 && jlIndex < dwCount) {
			...
			env->ThrowNew(rt.ioobEx, "index out of range");
			return NULL;
		}
	} else {
